...
public class PvPMoveLogger : MonoBehaviour
{
    ...
    private void StartListeningForOpponentMoves()
    {
        ...
        try
        {
            ...
            isListening = true;
            UnityEngine.Debug.Log("[PvPMoveLogger]  Started listening for opponent moves");
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Update board state on server after move
    /// </summary>
    /// <param name="fromPosition">Position unit moved from</param>
    /// <param name="toPosition">Position unit moved to</param>
    private async Task UpdateBoardState(Vector2Int fromPosition, Vector2Int toPosition)
    {
        try
        {
            ...
            // Update original cell to be empty
            ...
            // Update new cell with unit details
            ...
            UnityEngine.Debug.Log($"[PvPMoveLogger] Board state update completed: {fromPosition} -> {toPosition}");
        }
        catch (Exception ex)
        {
            UnityEngine.Debug.LogError($"[PvPMoveLogger] Error updating board state: {ex.Message}");
        }
    }
