...
public class AIPlayerHardController : AIPlayerController
{
    ...
    protected override IEnumerator PerformAIAction()
    {
        ...
        {
            UnityEngine.Debug.Log("🤖 No valid moves found. Ending turn.");
            TurnManager.Instance?.EndTurn();
        }
    }
    ...
    private IEnumerator ExecuteMoveSequence(RPSUnit unit, Vector2Int target)
    {
        ...
        {
            UnityEngine.Debug.Log($"🚫 Invalid move: {unit.name} tried to move more than one step from {unit.Position} to {target}");
            TurnManager.Instance?.EndTurn();
            yield break;
        }
        ...
        {
            UnityEngine.Debug.Log($"🚫 Cannot attack: Target unit is at {enemyUnit.Position} but move is to {target}");
            TurnManager.Instance?.EndTurn();
            yield break;
        }
        ...
        {
            UnityEngine.Debug.Log($"💥 {unit.name} stepped on trap!");
        ...
        {
            UnityEngine.Debug.Log($"🎯 {unit.name} captured the FLAG! YOU LOSE!");
        ...
        {
            UnityEngine.Debug.Log($"🤝 Tie – starting battle panel between {unit.name} and {enemyUnit.name} at {target}");
        ...
        {
            UnityEngine.Debug.Log($"🏆 {unit.name} wins! {unit.Kind} beats {enemyUnit.Kind}");
            BoardManager.Instance.RemoveUnit(enemyUnit);
            Destroy(enemyUnit.gameObject);
            unit.MoveTo(target);
            yield return new WaitForSeconds(0.6f);
        ...
        {
            UnityEngine.Debug.Log($"💀 {unit.name} loses the battle at {target}: {enemyUnit.Kind} beats {unit.Kind}");
            BoardManager.Instance.RemoveUnit(unit);
            Destroy(unit.gameObject);
            TurnManager.Instance?.EndTurn();
            yield break;
